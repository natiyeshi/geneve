...
const Addgallery = ({ setGallerys }: { setGallerys: Function }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/gallery", {
        method: "POST",
        body: JSON.stringify({ formData: { image: url,caption : caption.length < 1 ? "Image" : caption } }),
      });
      ...
      if (!res.ok) {
        ...
        const response = await res.json();
        setGallerys((test: IGallery[]) => [...test, response.newGallery]);
        setError(""); 
        setCaption("");
        setImage(null);
        ...
        setIsOpen(false);
        toast({
          description: "Gallerys Added Succesfully",
        });
      }
    } catch (err) {
